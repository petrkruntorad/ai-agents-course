...
class Grid2DEnv(gym.Env):
    def __init__(self):
        ...
        # Define traps as an array of positions
        self.trap_positions = [
    ...
    def step(self, action):
        ...
        print(
            f"Step: {self.step_counter} ---> Action: {action}={action_text}, State: {self.state}, Goal: {self.goal_position}")
        ...
        elif any(np.array_equal(self.state, trap_pos) for trap_pos in self.trap_positions):
            reward = -1.0
            terminated = True  # Natural terminal state - Goal reached
            truncated = False  # Not a timeout
            print(f"❌ Ended up in a trap in {self.step_counter} steps!")
    ...
    def render(self, mode="human"):
        ...
        # Place traps dynamically
        for i, trap_pos in enumerate(self.trap_positions):
            tr, tc = trap_pos
            grid[tr][tc] = f"T{i+1}"  # Trap position
